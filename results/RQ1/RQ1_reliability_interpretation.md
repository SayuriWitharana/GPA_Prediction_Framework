...
Regenerated 2026-07-20 under the shared, unified `src/preprocessing.py`: CV columns from `src/rq1/stage1_cv.py`, pooled external columns computed directly (see script snippet referenced in the reconciliation note), per-group external columns from `src/rq1/stage2_external_bootstrap.py`.
...
by `results/scripts/rq1/stage1_summary_table.py`, so it can't drift out of date the
way a hand-typed table here would. Re-run
`python -m results.scripts.rq1.stage1_summary_table` after any `stage1_cv.py` re-run.
...
Regenerated by `src/rq1/stage1_cv.py`. RMSE SD (fold-to-fold stability) is now included per group, since it is the criterion that actually determines each group's Stage-1 pass point in the three-criterion framework (`Final_Submission_Extended_Abstract.docx`), not just RMSE/bias.
...
RMSE/bias regenerated by `src/rq1/stage2_external_bootstrap.py` (which now uses the shared, unified preprocessor) with 95% bootstrap confidence intervals. **Full RMSE/bias + CI, all seven checkpoints S0-S6: see `results/RQ1/external_summary_table.md`** (auto-generated from `external_bootstrap_ci.csv` by `results/scripts/rq1/stage2_summary_table.py`, one table per group). The table below is kept only because it's the one place R² lives; **R² has not been recomputed** since the preprocessing unification — it's retained from the pre-unification run for reference only and should be treated as approximate.
...
**Updated 2026-08-24:** S0/S1 rows are omitted from the table above only because it predates this change — `src/rq1/stage2_external_bootstrap.py` was scoped to S2-S6 to match the paper's Table III (there was never a technical reason S0/S1 couldn't be bootstrapped too), and now covers all seven checkpoints. S0/S1 aren't backfilled into *this* table because it's never had an R² column to begin with (`bootstrap_group()` only ever computed RMSE and bias — the R² column above comes from a separate, pre-unification run) — mixing a live RMSE/bias with that stale R² source would be worse than the gap it fills. Live S0/S1 RMSE/bias + CIs are in `external_summary_table.md` above. If external R² is needed for S0/S1, that's a real, separate gap: `bootstrap_group()` needs an R² computation added, not just a table edit.
...
- `src/rq1/stage1_cv.py` and `src/rq1/stage2_external_bootstrap.py` are now the reproducible, unified-preprocessing source of truth for every number in this document (run with `python -m src.rq1.stage1_cv` / `python -m src.rq1.stage2_external_bootstrap`). Outputs: `cv_results.csv` and `external_bootstrap_ci.csv` in this folder.
