...
**Note (this file is regenerated by `src/rq2/rq2_group_shap.py`):** this fairness note is not written by the script and will be lost if the script is re-run again. If you re-run the RQ2 pipeline in the future, re-add this section (or move it into the script's `write_text` block so it survives regeneration).
